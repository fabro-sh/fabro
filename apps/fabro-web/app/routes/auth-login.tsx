...
export default function AuthLogin() {
  ...
  const hasSourceControlAuth = hasGitHub || hasGitLab;
  ...
  return (
    <AuthLayout>
      ...
      <p className="mt-3 text-center text-sm/6 text-fg-3 text-pretty">
        {hasGitHub && hasGitLab
          ? "Authenticate with your source control account to continue."
          : hasGitHub
            ? "Authenticate with your GitHub account to continue."
            : hasGitLab
              ? "Authenticate with your GitLab account to continue."
          : hasDevToken
      ...
      <div className="mt-6 space-y-4">
        ...
        {hasDevToken && hasSourceControlAuth ? (
          <DevTokenCollapsible
            token={token}
            setToken={setToken}
            error={error}
...
function GitLabMark() {
  return (
    <svg viewBox="0 0 16 16" fill="currentColor" className="size-4 shrink-0" aria-hidden="true">
      <path d="M8 15.42 2.86 9.6.48 2.34a.38.38 0 0 1 .71-.28L3.57 6.6h8.86l2.38-4.54a.38.38 0 0 1 .71.28L13.14 9.6 8 15.42Z" />
      <path d="M8 15.42 3.57 6.6h8.86L8 15.42Z" opacity="0.65" />
    </svg>
  );
}
