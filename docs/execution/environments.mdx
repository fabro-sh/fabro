...
<Warning>
The exe.dev sandbox provider is in **beta**. Its configuration surface and behavior may change in future releases.
</Warning>
...
- **VM lifecycle** — On `initialize()`, Arc connects to the exe.dev management plane via SSH and runs `new --json` to create a VM. The response contains the VM name and an SSH destination (e.g. `my-vm.exe.xyz`). On `cleanup()`, Arc runs `rm <vm_name>` to destroy the VM.
- **Working directory** — Fixed at `/home/exedev`. Relative paths resolve against this directory.
- **Commands** — Executed via SSH on the data plane (`<vmname>.exe.xyz`). Environment variables and working directory overrides are supported.
...
The exe.dev provider does not currently have additional configuration options.
...
| `exe` | Full machine | A cloud VM with an empty working directory at `/home/exedev`. The host filesystem is completely inaccessible. |
