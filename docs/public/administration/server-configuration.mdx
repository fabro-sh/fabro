...
[server.auth]
methods = ["dev-token", "github", "gitlab"]
...
| Key | Description | Default |
|---|---|---|
| `methods` | Ordered list of enabled bootstrap methods: `dev-token`, `github`, `gitlab` | `["dev-token"]` |
...
When `"gitlab"` is enabled, browser users can sign in with GitLab OAuth and receive a session cookie.
...
### `[server.auth.gitlab]` section

GitLab-specific auth policy.

| Key | Description |
|---|---|
| `allowed_usernames` | GitLab usernames allowed to complete OAuth login |
| `allowed_groups` | GitLab group `full_path` values whose members may complete OAuth login |

An empty GitLab allowlist does not block server startup, but no GitLab user can sign in until at least one username or group is configured. The GitLab OAuth client ID still lives under `[server.integrations.gitlab].client_id`.
...
### `[server.integrations.gitlab]` section

Configure GitLab integration auth. `strategy = "token"` uses the vault secret `GITLAB_TOKEN` for clone, push, branch checks, and merge request operations. `strategy = "app"` enables GitLab OAuth login and also requires `GITLAB_TOKEN` for repository automation. See the [GitLab integration guide](/integrations/gitlab) for the setup flow and OAuth redirect URI.
...
For GitLab app mode, set `strategy = "app"` and include `base_url` and `client_id`:
...
GitLab app mode requires `GITLAB_APP_CLIENT_SECRET` and `GITLAB_TOKEN` in the server vault. This differs from GitHub app mode: Fabro uses GitLab OAuth for human identity, but repository work still uses the configured automation token.

You can reconfigure GitLab from an existing local install with interactive prompts:

```bash
fabro install gitlab
```

For scripted environments, pass the GitLab inputs non-interactively:
