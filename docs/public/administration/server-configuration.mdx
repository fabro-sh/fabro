...
Configure GitLab integration auth. `strategy = "token"` uses the vault secret `GITLAB_TOKEN` for clone, push, branch checks, and merge request operations. `strategy = "app"` enables GitLab OAuth login and also requires `GITLAB_TOKEN` for repository automation.
