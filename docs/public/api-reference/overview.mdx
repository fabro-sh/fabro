...
When `"gitlab"` is enabled, browser users can sign in through GitLab OAuth. GitLab auth allows users when their username is listed in `[server.auth.gitlab].allowed_usernames` or one returned GitLab group `full_path` is listed in `[server.auth.gitlab].allowed_groups`. Empty GitLab allowlists do not block startup, but no GitLab user can sign in until at least one allowlist entry exists.

### Browser Sessions

When `[server.web].enabled = true`, the server requires `SESSION_SECRET` and issues a private `__fabro_session` cookie after successful login. The cookie is session transport only; the underlying bootstrap method remains `dev-token`, `github`, or `gitlab`, and that provenance is preserved in run metadata.
