...
Provider auth is declared in `[llm.providers.<id>.auth]`. API-key providers use ordered `env:<NAME>` or `credential:<id>` refs plus `header = "bearer"` or `header = { custom = "Header-Name" }`. Header-only providers set `type = "header_only"` and put typed `{ env = "NAME" }`, `{ credential = "id" }`, or `{ literal = "value" }` values in `extra_headers`.

`agent_profile` controls profile-specific behavior such as project-memory filenames, CLI/ACP command selection, and native session routing. Valid values are `anthropic`, `openai`, and `gemini`; model-level values override provider-level values.

`billing_policy` controls usage-cost estimation. Use `openai`, `anthropic`, `gemini`, or `none`.
