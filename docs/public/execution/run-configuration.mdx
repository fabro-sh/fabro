...
Configure whether clone-based sandboxes clone the run's GitHub or GitLab origin before execution.
...
Request the configured GitLab automation token for workflow stages that need `GITLAB_TOKEN` inside the sandbox.
...
The token is injected only when requested and is not written to run manifests. The server must have `GITLAB_TOKEN` in its vault.
...
Automatically open a pull request or merge request when the workflow run completes successfully. GitHub origins require a [GitHub App](/integrations/github). GitLab origins require a configured [GitLab integration](/integrations/gitlab) with `GITLAB_TOKEN`.
...
| `auto_merge` | When `true`, enables provider auto-merge on the created pull request or merge request. Implies `draft = false` since providers do not allow auto-merge on drafts. The repository must have auto-merge enabled in provider settings. Default: `false`. |
| `merge_strategy` | Merge method when `auto_merge` is enabled: `squash` (default), `merge`, or `rebase`. |

Auto-PR creation only runs when the workflow completes successfully or partially successfully and the terminal diff is non-empty. Empty-diff runs complete normally and skip PR creation.

GitLab merge requests appear through Fabro's existing pull request routes, events, and UI labels. GitLab supports `merge` and `squash` auto-merge strategies in Fabro; `rebase` is GitHub-only.
