...
| `server.auth.methods` | Bootstrap auth methods: `dev-token`, `github`, `gitlab`, or a combination |
...
- **`gitlab`** — GitLab OAuth for browser users, resulting in a session cookie when the user matches an allowed username or group
