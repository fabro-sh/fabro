...
- **`gitlab`** — End users sign in through GitLab OAuth and receive a browser session cookie.
...
For GitLab, use `methods = ["gitlab"]` or include it beside other methods. GitLab auth allows users when their username is listed in `server.auth.gitlab.allowed_usernames` or one returned GitLab group `full_path` is listed in `server.auth.gitlab.allowed_groups`. Empty GitLab allowlists do not block startup, but no GitLab user can sign in until at least one allowlist entry exists.
