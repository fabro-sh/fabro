...
| Key | Type / values | Default | Description |
|---|---|---|---|
| `auto_merge` | boolean | false | Enable provider auto-merge for created pull requests or merge requests.<br />Implies `draft = false`. |
...
| Key | Type / values | Default | Description |
|---|---|---|---|
| `token` | boolean | false | Opt in to injecting `GITLAB_TOKEN` at execution time. The token is not<br />written to run manifests. |
