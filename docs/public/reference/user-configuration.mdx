...
| `small_default` | boolean | `false` | Whether this model should be preferred for small/cheap utility tasks such as generated run titles. If no model is marked, Fabro falls back to the provider default. Set `false` in a higher-precedence layer to clear an inherited marker. |
