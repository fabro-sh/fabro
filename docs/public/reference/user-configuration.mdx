...
| Shared run defaults | `[run.model]`, `[run.environment]`, `[environments.<slug>]`, `[run.checkpoint]`, `[run.inputs]`, `[run.prepare]`, `[run.pull_request]`, `[run.integrations.github.permissions]`, `[run.integrations.gitlab]`, `[run.hooks]`, `[run.agent.mcps]` |
...
| Key | Type / values | Default | Description |
|---|---|---|---|
| `auto_merge` | boolean | false | Enable provider auto-merge for created pull requests or merge requests. Implies `draft =<br />false`. |
...
For GitLab merge requests, `rebase` is not supported; use `merge` or `squash`.

## `[run.integrations.gitlab]`

Request the configured GitLab automation token for workflow command and agent stages.
...
| Key | Type / values | Default | Description |
|---|---|---|---|
| `token` | boolean | false | Opt in to injecting `GITLAB_TOKEN` at execution time. The token is not written to run manifests. |
