...
| `fidelity` | How much prior context is passed to this node (see table below) |
...
Fidelity can also be set at the graph level (`default_fidelity`) or on individual edges to control the transition between stages.
...
| `max_visits` | Maximum times this node can execute in a run (prevents infinite loops) |
