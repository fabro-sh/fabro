...
impl Session {
    ...
    pub async fn initialize(&mut self) -> Result<(), Error> {
        ...
        let provider_profile_name = self.provider_profile.profile_kind().to_string();
        ...
        let memory_files: Vec<MemoryFileSummary> = self
            ...
            .collect();
        let total_loaded_bytes: usize = self.memory.iter().map(|doc| doc.loaded_bytes).sum();
        self.event_emitter
            .emit(self.id.clone(), AgentEvent::MemoryLoaded {
                provider_profile: provider_profile_name.clone(),
                files: memory_files,
                total_loaded_bytes,
...
mod tests {
    ...
    async fn initialize_emits_memory_loaded_with_file_metadata() {
        ...
        while let Ok(envelope) = rx.try_recv() {
            ...
            {
                memory_event = Some((files, total_loaded_bytes, budget_bytes, provider_profile));
                break;
            }
        }
        let (files, total_loaded_bytes, budget_bytes, provider_profile) =
            memory_event.expect("MemoryLoaded should be emitted");
        assert_eq!(provider_profile, "anthropic");
        assert_eq!(budget_bytes, 32768);
        assert_eq!(total_loaded_bytes, "Hello world".len());
