...
fn api_status_from_workflow(
    ...
    _reason: Option<WorkflowStatusReason>,
) -> RunStatus {
    match status {
...
fn update_live_run_from_event(state: &Arc<AppState>, run_id: RunId, event: &RunEvent) {
    ...
    match &event.body {
        ...
        | EventBody::InterviewInterrupted(_) => {
            // Return to Running if no pending interviews remain.
            // We check the durable projection lazily; for the live model,
            // assume the run returns to Running on any interview resolution.
...
"#,
    ...
    async fn cancel_queued_run_succeeds() {
        ...
        // Cancelled runs are off-board (not mapped to any column)
        let req = Request::builder()
            .method("GET")
            .uri(api("/boards/runs"))
            .body(Body::empty())
    ...
    async fn pause_run_sets_pending_control_on_board_response() {
        ...
        // Submitted status is off-board (no board column for Submitted/Queued/Starting)
        let req = Request::builder()
            .method("GET")
            .uri(api("/boards/runs"))
            .body(Body::empty())
        ...
        let item = body["data"]
            ...
            .iter()
            .find(|item| item["id"].as_str() == Some(run_id_str.as_str()));
        assert!(item.is_none(), "submitted run should not be on the board");
    }
...
"#,
    ...
    async fn boards_runs_returns_run_list_items_with_board_columns() {
        ...
        // Create a durable run in Running status so it appears on the board
        create_durable_run_with_events(
            &state,
            run_id,
            &[
