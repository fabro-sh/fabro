        ...
        Some("command") => {
            ...
            if let Some(stdout_val) = outcome.context_updates.get(keys::COMMAND_OUTPUT) {
                ...
                if stdout.trim().is_empty() {
                    ...
                    lines.push(format!(
                        "    {}",
                        tail_lines(stdout.trim(), COMPACT_OUTPUT_MAX_LINES)
                    ));
                    lines.push("    ```".to_string());
                }
            }
            if let Some(stderr_val) = outcome.context_updates.get(keys::COMMAND_STDERR) {
                ...
                if stderr.trim().is_empty() {
                    ...
                    lines.push(format!(
                        "    {}",
                        tail_lines(stderr.trim(), COMPACT_OUTPUT_MAX_LINES)
                    ));
                    lines.push("    ```".to_string());
                }
            }
            ...
            if let Some(stdout_val) = outcome.context_updates.get(keys::COMMAND_OUTPUT) {
                if let Some(path) = artifact_path(stdout_val) {
                    ...
                    if stdout.trim().is_empty() {
                        ...
                        lines.push(format!(
                            "  {}",
                            tail_lines(stdout.trim(), SUMMARY_HIGH_OUTPUT_MAX_LINES)
                        ));
                        lines.push("  ```".to_string());
                    }
                }
            ...
            if let Some(stderr_val) = outcome.context_updates.get(keys::COMMAND_STDERR) {
                if let Some(path) = artifact_path(stderr_val) {
                    ...
                    if stderr.trim().is_empty() {
                        ...
                        lines.push(format!(
                            "  {}",
                            tail_lines(stderr.trim(), SUMMARY_HIGH_OUTPUT_MAX_LINES)
                        ));
                        lines.push("  ```".to_string());
                    }
                }
