...
- failure_signature: verify|transient_infra|script failed with exit code: <n> ## stdout error: variables can be used directly in the `format!` string --> lib/crates/fabro-cli/src/commands/run/wait.rs:<n>:<n> | <n> | / bail!( <n> | | "run '{}' has no status record yet. try again in a 
